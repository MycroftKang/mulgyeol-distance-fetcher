...
class Mecro(QtCore.QObject):
    ...
    def naver_get_point_url(self, query):
        ...
        url = 'https://m.map.naver.com/apis/search/poi?query={}&page=1'.format(
            query)
        return url
    ...
    def naver_get_distance_url(self, start_point, end_point):
        ...
        url = 'https://m.map.naver.com/spirra/findCarRoute.nhn?route=route3&output=json&result=web3&coord_type=latlng&search={}&car=0&mileage=12.4&start={}&destination={}'.format(
            self.option_to_naver_id[self.option], start_point, end_point)
        return url
    ...
    def naver_get_point(self, query, return_name=False):
        ...
        try:
            ...
            self.logger.error(
                ('[Code P] 400 Bad Request or 404 Not Found'+query+'\n' + r.text), exc_info=True)
            raise ValueError
        if return_name:
    ...
    def kakao_get_point_url(self, query):
        ...
        url = 'https://search.map.daum.net/mapsearch/map.daum?callback=jQuery{}_{}&q={}&msFlag=A&sort=0&gb=R'.format(
            expando, timestamp, query)
        return url
    ...
    def kakao_get_distance_url(self, start_point, start_id, end_point, end_id):
        ...
        expando = '181' + str(random.random()).replace('.', '')
        timestamp = str(round(time.time(), 3)).replace('.', '')
        url = 'https://map.kakao.com/route/carset.json?roadside=ON&sp={},ADDRESS,{}&ep={},ADDRESS,{}&callback=jQuery{}_{}&carMode={}&carOption=NONE'.format(
            start_point, start_point, end_point, end_id, expando, timestamp, self.option_to_kakao_id[self.option])
        return url
    ...
    def kakao_get_point(self, query, return_name=False):
        ...
        try:
            ...
            self.logger.error(
                ('[Code P] 400 Bad Request or 404 Not Found\n' + res), exc_info=True)
            raise ValueError
    ...
    def distance_query(self, query, platform):
        ...
        if platform == 0:
            try:
                ...
                self.logger.warning(
                    ('[Code Q1] Input Address not Exist::' + query), exc_info=True)
                distance = 0
                error = True
                return (error, distance)
            except:
                self.logger.error(
                    '[Code Q1] 400 Bad Request\n'+query, exc_info=True)
                raise PermissionError

            try:
                r = self.request_to_naver(
                    self.naver_get_distance_url(start_point, end_point))
                r.raise_for_status()
                res = r.text
            ...
            try:
                r = self.request_to_kakao(self.kakao_get_distance_url(
                    start_point, start_id, end_point, end_id))
                r.raise_for_status()
                res = re.findall('({.*})', r.text, re.DOTALL)
                res_dict = json.loads(res[0])
    ...
    def save_result(self, error, distance):
        if self.operation <= self.max_row:
            if not error:
                self.sheet[self.column2 +
                           str(self.operation)] = distance / 1000
    ...
    def fetch(self):
        ...
        try:
            if self.config_list[0] == 0:
                self.destination_point, name = self.naver_get_point(
                    self.destination, True)
            else:
                self.destination_point, self.destination_id, name = self.kakao_get_point(
                    self.destination, True)
        except ValueError:
        ...
        for i in range(self.total):
            if self.online:
                ...
                if i != 0:
                    rand = random.uniform(
                        self.config_list[1], self.config_list[2])
                    time.sleep(rand)
                if not self.online:
                    print('call break')
                    break
                ...
                if not load_error:
                    try:
                        fetch_error, distance = self.distance_query(
                            starting_point, self.config_list[0])
                    except Exception as e:
                        self.logger.critical(
                            ('[Code F1] Network Error::' + str(e)), exc_info=True)
                    ...
                    try:
                        ...
                        self.logger.critical(
                            ('[Code F2] Permission Denied::' + str(e)), exc_info=True)
                    ...
                    last_time = (run_time + (self.config_list[1] + self.config_list[2]) / 2) * \
                        last_item + \
                        last_item // self.config_list[3] * \
                        self.config_list[4] * 60
                    emit_data = [self.total, starting_point, distance /
                                 1000, last_time, last_item, self.cur, name]
                    if last_percent == 0:
