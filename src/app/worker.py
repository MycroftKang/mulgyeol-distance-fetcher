...
class Worker(QtCore.QObject):
    ...
    def __init__(self):
        ...
        self.fetcher.percentChanged.connect(
            self.get_percent_signal, QtCore.Qt.QueuedConnection)
        self.fetcher.completion.connect(
            self.label_signal.emit, QtCore.Qt.QueuedConnection)
        self.fetcher.permission.connect(
            self.permission_Signal.emit, QtCore.Qt.QueuedConnection)
        self.fetcher.waitsignal.connect(
            self.wait_signal.emit, QtCore.Qt.QueuedConnection)
    ...
    def start_work(self):
        ...
        self.fetcher.setValue(config_list, self.file_address, self.target, self.sheet_name,
                              self.addr_column, self.distance_column, self.option, self.start_row, self.end_row)
        print('pass')
        self.fetcher.fetch()
